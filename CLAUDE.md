...
- **Сервер** — Node.js + Express + Socket.IO (`server/`). `rooms.js` —
  ...
  через `FINISH_GRACE_MS` после первого финишёра гонка принудительно
  завершается — фоновая вкладка браузера замораживает игру игрока, и без
  таймаута он подвесил бы гонку всем).
